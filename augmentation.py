...
class Augment_image:
    ...
    def flip(self, img, mask):
        ...
        img = transforms.Resize((self.image_size, self.image_size), interpolation=InterpolationMode.BILINEAR)(img)
        mask = transforms.Resize((self.image_size, self.image_size), interpolation=InterpolationMode.NEAREST)(mask)
        return img, mask

    # 随机旋转  1~360°随机旋转
    def rotate(self, img, mask):
        random_angle = transforms.RandomRotation.get_params([-30, 30])
        img = F.rotate(img, random_angle, fill=0)
        mask = F.rotate(mask, random_angle, fill=0)
    ...
    def random_resize_crop(self, img, mask, scale=(0.6, 1.0), ratio=(1, 1)):
        ...
        img = F.resized_crop(img, top, left, height, width, (self.image_size, self.image_size),
                              interpolation=InterpolationMode.BILINEAR)
        mask = F.resized_crop(mask, top, left, height, width, (self.image_size, self.image_size),
    ...
    def affine(self, img, mask):
        if random.random() >= 0.5:
            ...
            # 0值填充，原始图像大小
            img = F.perspective(img, startpoints, endpoints, interpolation=Image.BICUBIC, fill=0)
            mask = F.perspective(mask, startpoints, endpoints, interpolation=InterpolationMode.NEAREST, fill=0)
        elif random.random() < 0.5:
            ...
            # 0值填充，原始图像大小
            img = F.affine(img, *ret, interpolation=0, fill=0)
            mask = F.affine(mask, *ret, interpolation=0, fill=0)
...
class Train_Transform(object):
    ...
    def __call__(self, img, mask):
        ...
        transform_list = [methodcaller('random_resize_crop', img, mask), methodcaller('resize', img, mask)
            ...
            , methodcaller('colof_jitter', img, mask), methodcaller('flip', img, mask)
            , methodcaller('random_resize_crop', img, mask)]
