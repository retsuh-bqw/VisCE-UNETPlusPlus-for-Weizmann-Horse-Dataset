...
# 获取标签和预测的边界iou
def calc_boundary_iou(gt, dt, dilation_ratio=0.02):
...
# 损失函数
def BCE_DiceCoef_loss(predict_batch, mask_batch):
    smooth = 1e-5
    # bce loss
    squeeze_predict_batch = predict_batch.squeeze(dim=1)
    bce_loss = F.binary_cross_entropy_with_logits(squeeze_predict_batch, mask_batch)
    # dice loss
    logits_pred = torch.sigmoid(squeeze_predict_batch)
    num = mask_batch.size(0)
    logits_pred = logits_pred.view(num, -1)  
    mask_batch = mask_batch.view(num, -1)  
    intersection = (logits_pred * mask_batch)
    dice_coef = (2. * intersection.sum(1) + smooth) / (logits_pred.sum(1) + mask_batch.sum(1) + smooth)
    dice_loss = 1 - dice_coef.sum() / num
